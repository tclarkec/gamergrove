...
const App: React.FC<AppProps> = (props: AppProps) => {
  ...
  const handleOnContextMenu = (e: any): void => {
    ...
    const x: number = Math.min(Math.max(menuRadius + 10, e.clientX - itemRadius), window.innerWidth - (menuRadius * 1.5) - 10),
          y: number = Math.min(Math.max(menuRadius + 10, e.clientY - itemRadius), window.innerHeight - (menuRadius * 1.5) - 10);
